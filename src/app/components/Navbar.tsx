...
function Navbar({ isHome = false, text = "Lorem Ipsum", anchor = "#"}: { isHome?: boolean, text?: string, anchor?: string }) {
    ...
    : <a href={anchor} title="Back" className='p-1'><span className="pi pi-arrow-left 
    ...
    const title_page = (
        <div className="max-w-full w-screen justify-content-center z-0">
            <div className="text-center text-2xl font-bold uppercase" style={{color: "#fff"}}>{text}</div>
        </div>
    )
    ...
    return (
        <>
            ...
            (<Menubar start={start} model={item} style={{backgroundColor: "#060153"}} className="px-3 py-2" />)
            }
        </>
    )
}
