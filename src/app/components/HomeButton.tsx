...
export default function HomeButton({text, icon, inverted = false, backgroundImage, link = "#"}: {text: string, icon: string, inverted: boolean, backgroundImage: string, link: string}) {
    ...
    const textComponent = <div className={`${!inverted ? 'pl-2' : 'pr-2'} text-white uppercase`}  style={{fontSize: "1.15rem"}}>{text}</div>;
    const iconComponent = <i className={`${icon} ${ !inverted ? 'pr-2' : 'pl-2'}`} style={{ fontSize: '2.25rem', color: "#2A21BC"}}></i>;
